...
class _com_io {
   ...
   public:
    ...
    _com_io(_com_io &&a) { *this = std::move(a); }
    ~_com_io() { suicide(); }
    ...
    std::string recv(std::size_t sz, int timeout_sec) const {
        ...
        while (sz) {
            ...
            // }
            int status = select(rfd + 1, &fds, nullptr, nullptr, &tv);
            if (status == -1) {
                throw std::runtime_error("AGENT::recv::select() error\n");
            } else if (status == 0) {
            ...
            int elapsed =
                1e6 * std::chrono::duration_cast<std::chrono::duration<double>>(t1 - t0)
                          .count();
    ...
    while (stay) {
        ...
        switch (c) {
            ...
            case 'z':
                ...
                if (b.winner() != Color::OTHER) {
                    b.no_hl = 1;
                }
                num = ml[now_move].first;
                dir = ml[now_move].second;
                break;
            case 'U':
            case 'u':
                if (b.history.size() < 3 and undo_twice) {
                    std::cout << "\a";
                    continue;
                }
                stay = 0;
            ...
            case '\033':
                ...
                now_move =
                    (now_move + ((c == 'B' || c == 'C') ? 1 : ml.size() - 1)) % ml.size();
